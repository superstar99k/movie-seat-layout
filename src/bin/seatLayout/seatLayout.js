...
(function ($) {
    function Movieseat(_paramObject) {
        ...
        var getMainLayoutFunc = function (_obj) {
            ...
            _html += "<div class='movie-screen " + pluginOptions.classes.screen + "'> 舞台 </div>";
        ...
        getSeatLayoutFunc = function (_obj, isSeat, _row, _area) {
            ...
            if (_obj) {
                ...
                if (isSeat) {
                    ...
                    return "<li data-seatdefination='" + dataString + "' class='seat-row-seat seat-yes " + classes + " ' title='" + `${_obj.PhyRowId}${_obj.seatNumber} (${_obj.seatRank}席)` + "'><span></span></li>";
                } else {
    ...
    $.fn.seatLayout = function (_options) {
        ...
        _html = seatInstance.getMainLayout(_optionsObj.data.seatLayout);
        _el.html(_html);
        ...
        _el.find('li').click(function (e) {
            if ($(this).hasClass('can-select')) {
                ...
                if (!$(this).hasClass('current-selected')) {
                    ...
                    const seat = getObjData($(this));
                    selectedSeats = selectedSeats.filter(item => !(
                        item.PhyRowId == seat.PhyRowId &&
                ...
                var dataToPass = $.extend({}, getObjData($(this)));
                dataToPass.selected = selectedSeats;
                _optionsObj.callOnSeatSelect(e, dataToPass, selectedSeats, this);
            }
        });
